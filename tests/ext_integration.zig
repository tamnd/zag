...
// The fixtures module is generated by tests/fixtures/ext/gen.sh, which
// build.zig runs before this module compiles. Adding a new fixture
// means dropping an NN_foo.py into tests/fixtures/ext/ and re-running the
// test -- no change here.
const fixtures = @import("ext_fixtures");

test "ext fixtures match CPython output" {
    inline for (fixtures.names) |name| {
        runFixture(
            name,
            @embedFile("fixtures/ext/" ++ name ++ ".cpython-314.pyc"),
            @embedFile("fixtures/ext/" ++ name ++ ".expected.txt"),
        ) catch |err| {
...
fn runFixture(name: []const u8, pyc: []const u8, expected: []const u8) !void {
    ...
    inline for (fixtures.names) |fname| {
        if (fname.len > 0 and fname[0] == '_') {
            const helper_pyc = @embedFile("fixtures/ext/" ++ fname ++ ".cpython-314.pyc");
    ...
    inline for (fixtures.helpers) |h| {
        const helper_pyc = @embedFile("fixtures/ext/" ++ h.path);
        const helper_code = try zag.marshal.pyc.loadBytes(run_alloc, helper_pyc);
